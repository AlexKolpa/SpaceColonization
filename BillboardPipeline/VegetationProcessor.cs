...
namespace BillboardPipeline
{
    ...
    public class VegetationProcessor : ModelProcessor
    {
                                             ...
                                             ContentProcessorContext context)
        {
            ...
            IList<Vector3> treePos = GenerateVegetation(input, input.Identity);

            // Chain to the standard ModelProcessor.
            ModelContent model = base.Process(input, context);
           
            //add tree positions as tag
            model.Tag = new TreePositionContent(treePos);
        ...
        /// </summary>
        IList<Vector3> GenerateVegetation(NodeContent node, ContentIdentity identity)
        {
            List<Vector3> childTreePos = new List<Vector3>();
            // First, recurse over any child nodes.
            foreach (NodeContent child in node.Children)
            {
                 childTreePos.AddRange(GenerateVegetation(child, identity));
            }
                    ...
                    for (int triangle = 0; triangle < indices.Count; triangle += 3)
                    {
                        ...
                        for (int count = 0; count < BillboardsPerTriangle; count++)
                        {
                            ...
                            else if (random.NextDouble() < CatProbability)
                            {
                                ...
                                GenerateBillboard(vegetationMesh, cats, position, normal);
                            }
                            else
                            {
                                // Add a new billboard to the output geometry.
                                GenerateBillboard(vegetationMesh, grass, position, normal);
                            }
